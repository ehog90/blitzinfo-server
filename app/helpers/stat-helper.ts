...
export module StatHelper {
    export function getFlatTenMinStatistics(result): any[] {
        return result.map(elem => {
            const countryStat = _(elem.data).toPairs().orderBy([x => x[1], x => x[0]], ['desc', 'asc']).value();
    ...
    export function processStatResult(result: any): any[][] {
        const arrayResult = _(result).toPairs()
            .map(x => [x[0], x[1]['c'], x[1]['l'], 0]).orderBy([x => x[1], x => x[0]], ['asc', 'asc']).value();
