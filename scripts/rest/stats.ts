...
import {IAllStatDocument} from "../interfaces/entities";
...
export function tenminStats(req: express.Request, res: express.Response): any {
    ...
    mongo.TenminStatMongoModel
        .find({}).sort({ timeStart: -1 }).limit(time * 6).lean().exec((error, result) => {
...
export function currentUTCYearStats(req: express.Request, res: express.Response) {
    ...
    mongo.AllStatMongoModel
        .findOne({ isYear: true, period: year.toString() }).lean().exec((error, result: IAllStatDocument) => {
...
export function overallStats(req: express.Request, res: express.Response) {
    mongo.AllStatMongoModel
        .findOne({ isYear: false, period: 'all' }).lean().exec((error, result: IAllStatDocument) => {
        res.end(JSON.stringify(StatUtils.processStatResult(result.data)));
    });
}
...
export async function lastMinutesStatistics(req: express.Request, res: express.Response) {
    ...
    const results = await mongo.MinStatMongoModel
        .find({ 'timeStart': { '$gt': new Date().getTime() - ((minutes + 1) * 60 * 1000) } }).lean();
