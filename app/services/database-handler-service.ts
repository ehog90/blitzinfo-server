...
class DatabaseHandlerService implements IDatabaseSaver {
  ...
  private saveStroke(stroke: IStroke) {
    ...
    const strokeToInsertTtlOneHour = new mongo.TtlOneHourStrokeMongoModel(
      stroke,
    );
  ...
  private setUpGeocoder(reverseGeoCoder: IReverseGeoCoderService) {
    ...
    this.reverseGeoCoder.lastGeocodedStroke.subscribe((x) =>
      this.strokeGeoCoded(x),
    );
  }

  private strokeGeoCoded(stroke: IStroke) {
    if (this.isDupeChecking) {
      mongo.TtlTenMinStrokeMongoModel.findOne({
  ...
  private async updateStatistics(savedStroke: IStroke) {
    const tenminTime = new Date(
      savedStroke.time.getTime() - (savedStroke.time.getTime() % 600000),
    );
    const minTime = new Date(
      savedStroke.time.getTime() - (savedStroke.time.getTime() % 60000),
    );
    ...
    update.$inc = inc;
    update[
      `data.${savedStroke.locationData.cc}.l`
    ] = savedStroke.time.getTime();
    ...
    await mongo.MinStatMongoModel.updateOne({ timeStart: minTime }, update, {
      upsert: true,
    });
