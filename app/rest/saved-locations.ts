...
export async function getLocationsForUser(
  ...
  const locResult = await mongo.SavedLocationMongoModel.find({
    uid: req.authContext._id,
  });
  res.json(locResult);
}

export async function newLocationInstance(
  req: INewSavedLocationInstance,
  res: express.Response,
) {
  const existingResult = await mongo.SavedLocationMongoModel.findOne({
    ...
    name: {
      $regex: new RegExp(
        ['^' + req.body.name.toLowerCase() + '$'].join(''),
        'i',
      ),
    },
  });
  if (!existingResult) {
    const result = await locationUpdaterInstance.reverseGeocodeWithCountryAsync(
      req.body.latLon,
    );
...
export async function removeLocationInstance(
  req: IRemoveSavedLocationInstance,
  res: express.Response,
) {
