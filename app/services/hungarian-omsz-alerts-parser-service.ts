import {loggerInstance as loggerInstance} from "./logger-service";
...
class HungarianOmszAlertsParserService {
            ...
            alertArea.alerts.forEach(async x => {
                ...
                if (result == null) {
                    ...
                    if (alertArea.type === HungarianAlertTypes.County) {
                        ...
                        await HungarianOmszAlertsParserService.notify(countyResults, alertArea);
                    } else {
    ...
    private async onTimerTick() {
        ...
        let ruResponses = await from(this.metHuData.regionalUnits).pipe(
            flatMap(ru => HungarianOmszAlertsParserService.getDataFromMetDotHu(ru, HungarianAlertTypes.RegionalUnit)),
            merge(4),
            reduce((acc, value) => {
                acc.push(value);
                return acc;
