...
export class GoogleReverseGeocoder implements IReverseGeoCoderAsync {
  ...
  public async getGeoInformation(latLonPair: number[]): Promise<IGeoAddress> {
    try {
      ...
      const geoCodingResponse = await getHttpRequestAsync<IGoogleGeocodingResponse>(
        url,
        5000,
      ).toPromise();
      ...
      if (geoCodingResponse.results.length > 0) {
        ...
        geoCodingResponse.results.forEach((result) => {
          result.address_components.forEach((resultElem) => {
            if (
              resultElem.types.indexOf('route') !== -1 &&
              !locationData.strDef
            ) {
              locationData.strDef = resultElem.long_name;
            } else if (
              resultElem.types.indexOf('locality') !== -1 &&
              !locationData.smDef
            ) {
              locationData.smDef = resultElem.long_name;
            } else if (
              resultElem.types.indexOf('country') !== -1 &&
              !locationData.cc
            ) {
              ...
              resultElem.types.indexOf('neighborhood') !== -1 &&
              !locationData.suburbDef
            ) {
              locationData.suburbDef = resultElem.long_name;
            } else if (
              resultElem.types.indexOf('street_number') !== -1 &&
              +streetNumber
            ) {
