...
export function getFlatTenMinStatistics(
  result: IMinutelyStatDocument[],
): any[] {
...
export function getFlatAllStatistics(
  results: IMinutelyStatDocument[],
): any[][] {
  const allCountryData = _(results)
    .flatMap((x) => x.data)
    .reduce(
      (acc, curr) =>
        _.mergeWith(acc, curr, (obj, src) => {
          if (!obj) {
            return src;
          }
          obj.c += src.c;
