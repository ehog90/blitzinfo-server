...
import {
  ...
  SunData,
  UserData,
} from './mongoose-primitive-scheme';
...
mongoStrokeSchemaTtlTenMin.plugin(MongooseAutoIncrementID.plugin, {
  modelName: 'ttlTenMinStrokes',
});
...
mongoStrokeSchemaTtlOneHour.plugin(MongooseAutoIncrementID.plugin, {
  modelName: 'ttlOneHourStrokes',
});
mongoStrokeSchemaTtlOneHour.index({ time: 1 }, { expireAfterSeconds: 3600 });

const mongoStrokeSchema = new mongoose.Schema(strokeSchema);
mongoStrokeSchema.plugin(MongooseAutoIncrementID.plugin, {
  modelName: 'allStrokes',
});
...
export const AllStrokeMongoModel = mongoose.model<IStrokeDocument>(
  'allStrokes',
  mongoStrokeSchema,
);
...
export const TenminStatMongoModel = mongoose.model<IMinutelyStatDocument>(
  'tenMinStat2',
  mongoTenminSchema,
);
export const MinStatMongoModel = mongoose.model<IMinutelyStatDocument>(
  'minStat2',
  mongoMinSchema,
);
export const AllStatMongoModel = mongoose.model<IAllStatDocument>(
  'allStat',
  mongoAllSchema,
);
...
export const LocationRecentMongoModel =
  mongoose.model<IDeviceLocationRecentDocument>(
    'locationDataRecent',
    mongoLocationRecent,
  );
export const SavedLocationMongoModel = mongoose.model<ISavedLocation>(
  'savedLocation',
  savedLocationsSchema,
);
export const AlertsMongoModel = mongoose.model<IMetHuAlertDocument>(
  'methuAlerts',
  mongoAlertsSchemaTtl,
);
export const LogsMongoModel = mongoose.model<ILogDocument>(
  'log',
  mongoLogsSchema,
);
export const StationsMongoModel = mongoose.model<IStationDocument>(
  'stations',
  mongoStationsSchema,
);
