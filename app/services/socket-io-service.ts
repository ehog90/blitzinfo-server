    ...
    private async onStatQueryRequested(request: StrokeSocket, message: any) {
        ...
        const statData = await forkJoin(
            mongo.AllStatMongoModel.findOne({
                isYear: false,
                period: 'all'
            }).toObservable().pipe(map(x => StatHelper.processStatResult(x.data))),
            mongo.AllStatMongoModel.findOne({
                isYear: true,
                period: year.toString()
            }).lean().toObservable().pipe(map((x: IAllStatDocument) => StatHelper.processStatResult(x.data))),
            mongo.MinStatMongoModel
                .find({
                    'timeStart':
                        {'$gt': new Date(Date.now() - SocketIoServer.LAST_DAY)}
                }).lean()
                .toObservable().pipe(map(x => StatHelper.getFlatAllStatistics(x))),
            mongo.MinStatMongoModel
                .find({
                    'timeStart':
...
export const socketIoService: ISocketIoServer = new
