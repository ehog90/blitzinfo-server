...
class DatabaseHandlerService implements IDatabaseSaver {
   ...
   private updateStatistics(savedStroke: IStroke) {
      ...
      mongo.TenminStatMongoModel.update({ timeStart: tenminTime }, { $inc: incAlone }, { upsert: true })
         .lean()
         .exec();
   ...
   private initializeTimer(): void {
      this.dupeCheckerTimeoutTimer = timer(
         config.dbDupeCheckingTimeout * 1000,
         config.dbDupeCheckingTimeout * 1000
      ).pipe(timeInterval());
      this.timerSubscription = this.dupeCheckerTimeoutTimer
         .pipe(take(1))
