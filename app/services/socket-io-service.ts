...
export class SocketIoServer implements ISocketIoServer {
  ...
  constructor(
    ...
    this.httpServer.on('error', (errorMessage) =>
    ...
    this.statUpdaterTimer = timer(0, this.statRefreshTickInSeconds * 1000).pipe(
      timeInterval(),
    );
  ...
  private static getLocaleName(stroke: IStroke, locale: string): IStroke {
    if (
      locale &&
      localeDb[locale] !== undefined &&
      stroke.locationData[`sm_${locale}`]
    ) {
  ...
  private logsReceived(log: ILog): void {
    this.getAllSockets().forEach((connection) => {
      if (
        connection.connectionType.indexOf(SocketIoConnectionTypes.Log) !== -1
      ) {
  ...
  private onConnectionRequest(request: StrokeSocket) {
    ...
    request.on(SocketIoRooms.StrokesInit, (message) =>
      this.onStrokesInitReceived(request, message),
    );
    request.on(SocketIoRooms.StatsInit, (msg) =>
      this.onStatQueryRequested(request, msg),
    );
    request.on(SocketIoRooms.LoggingInit, (message) =>
      SocketIoServer.onLogRequestReceived(request),
    );
  ...
  private onServerError(error: Error) {
    ...
    if (error.name === 'EADDRINUSE') {
      this.logger.sendErrorMessage(
        ...
        0,
        'Socket.IO Server',
        `Fatal error, the port is in use.`,
        false,
      );
      process.exit(1);
    }
  ...
  private async onStatQueryRequested(request: StrokeSocket, message: any) {
    this.logger.sendNormalMessage(
      ...
      'Socket.IO server',
      `Stat request: ${
        request.client.conn.remoteAddress
    ...
    try {
      ...
      if (this.validateInitializationMessage(initMessage)) {
        ...
        if (connection.userInfo !== undefined) {
          if (
            initMessage.lastRec.getTime() -
              connection.userInfo.lastRec.getTime() <
            500
          ) {
        ...
        initMessage.acc =
          initMessage.acc === undefined ? 0 : Number(initMessage.acc);
        ...
        if (canSendInitialData) {
          ...
          let result = this.getLocaleNames(
            dataWithDistances,
            connection.userInfo.lang,
          );
          ...
          if (result.length !== 0) {
            if (connection.userInfo.dtr === 0) {
              ...
              connection.emit(SocketIoRooms.Control, {
                distance: connection.userInfo.ad,
              });
            }
        ...
        if (connection.userInfo.cn) {
          ...
          const hungarianData = await this.locationUpdater.getHungarianData(
            connection.userInfo.latLon,
          );
  ...
  private queryOverallStats(isYear = false, period = 'all'): Promise<any> {
  ...
  private statUpdateTriggered(): void {
    this.getAllSockets().forEach((connection) => {
      if (
        connection.connectionType.indexOf(SocketIoConnectionTypes.Stat) !== -1
      ) {
  ...
  private strokeReceived(stroke: IStroke): void {
    this.getAllSockets().forEach((connection) => {
      if (
        connection.connectionType.indexOf(SocketIoConnectionTypes.Strokes) !==
        -1
      ) {
        if (connection.userInfo !== undefined) {
          ...
          if (
            ...
            (geoUtils.isInDirection(
              userInfo.dirs,
              clientGeoInformation.bearing,
            ) ||
              clientGeoInformation.distance <= 10)
          ) {
            const strokeLocalized = SocketIoServer.getLocaleName(
              stroke,
              connection.userInfo.lang,
            );
            const compressedStroke = flattenStroke(strokeLocalized);
            connection.emit(
              SocketIoRooms.Strokes,
              JSON.stringify(compressedStroke),
            );
          }
        }
      } else if (
        ...
        connection.emit(
          SocketIoRooms.Stats,
          JSON.stringify(toAllStatJson(stroke)),
        );
      }
    });
  }

  private validateInitializationMessage(
    initializationMessage: IInitializationMessage,
  ): boolean {
    if (
    ...
    initializationMessage.dirs.forEach((x) => !isNaN(x));
    return true;
  }
